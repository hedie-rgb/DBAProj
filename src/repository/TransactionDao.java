...
public class TransactionDao implements Repository<Transaction, Integer>{
	...
	public List<Transaction> findByIDs(Collection<Integer> ids) {
		...
		try {
			for(int id : ids) {
				users.add(em.find(Transaction.class, id));
			}
			return users;
		} catch (Exception e) {
			return null;
	...
	public List<Transaction> findAll() {
		Query q = em.createQuery("select * from Transactions");
	...
	public Boolean deleteByID(Integer id) {
		try {
			executeInsideTransaction(entityManager -> em.remove(em.find(Transaction.class, id)));
			return true;
		} catch(Exception e) {
			return false;
		}
	...
	public Transaction save(Transaction E) {
		try {
			...
			return em.find(Transaction.class, emf.getPersistenceUnitUtil().getIdentifier(E));
		} catch(Exception e) {
