...
public class DomesticDao implements Repository<Domestic, Integer>{
	...
	public Domestic findbyId(Integer id) {
		try {
			return em.find(Domestic.class, id);
		} catch (Exception e) {
			return null;
		}
	}
	...
	public List<Domestic> findByIDs(Collection<Integer> ids) {
		List<Domestic> users = new ArrayList<Domestic>();
		try {
			for(int id : ids) {
				users.add(em.find(Domestic.class, id));
			}
			return users;
		} catch (Exception e) {
			return null;
	...
	public List<Domestic> findAll() {
		Query q = em.createQuery("select * from Domestic");
	...
	public Boolean deleteByID(Integer id) {
		try {
			executeInsideTransaction(entityManager -> em.remove(em.find(Domestic.class, id)));
			return true;
		} catch(Exception e) {
			return false;
		}
	...
	public Domestic save(Domestic E) {
		try {
			...
			return em.find(Domestic.class, emf.getPersistenceUnitUtil().getIdentifier(E));
		} catch(Exception e) {
