...
public class RoomDao implements Repository<Room, Integer>{
	...
	public List<Room> findByIDs(Collection<Integer> ids) {
		...
		try {
			for(int id : ids) {
				users.add(em.find(Room.class, id));
			}
			return users;
		} catch (Exception e) {
			return null;
	...
	public List<Room> findAll() {
		Query q = em.createQuery("select * from Room");
	...
	public Boolean deleteByID(Integer id) {
		try {
			executeInsideTransaction(entityManager -> em.remove(em.find(Room.class, id)));
			return true;
		} catch(Exception e) {
			return false;
		}
	...
	public Room save(Room E) {
		try {
			...
			return em.find(Room.class, emf.getPersistenceUnitUtil().getIdentifier(E));
		} catch(Exception e) {
