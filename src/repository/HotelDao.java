...
public class HotelDao implements Repository<Hotel, Integer>{
	...
	public List<Hotel> findByIDs(Collection<Integer> ids) {
		List<Hotel> users = new ArrayList<Hotel>();
		try {
			for(int id : ids) {
				users.add(em.find(Hotel.class, id));
			}
			return users;
		} catch (Exception e) {
			return null;
	...
	public List<Hotel> findAll() {
		Query q = em.createQuery("select * from Hotel");
	...
	public Boolean deleteByID(Integer id) {
		try {
			executeInsideTransaction(entityManager -> em.remove(em.find(Hotel.class, id)));
			return true;
		} catch(Exception e) {
			return false;
		}
	...
	public Hotel save(Hotel E) {
		try {
			...
			return em.find(Hotel.class, emf.getPersistenceUnitUtil().getIdentifier(E));
		} catch(Exception e) {
